document.addEventListener("DOMContentLoaded", function() {
  ...
  function InstructionParsing(instruction) {
    let separator = instruction.charAt(0)
  ...
  UIElement.prototype.createEventListener = function() {
    let self = this
    let clicked = document.querySelectorAll(self.clickSelector)
    ...
    function clickCallback(e) {
      switch (self.targetSelector) {
        ...
        case undefined:
          ...
          break
        default:
          let target = document.querySelectorAll(self.targetSelector)
          let i = target.length
          while (i--) {
  ...
  codeBlockContent.split(delimiter).forEach(function(data) {
    if (!data) return
    let params = new InstructionParsing(data.trim())
    new UIElement(
